...
const decodeJwt = (token: string): DecodedJwt => {
  ...
  const [header, payload, signature] = parts.map((encodedPayload) =>
...
export const useAuthActions = () => {
  ...
  const handleAuthenticationLogin = async () => {
    await WebBrowser.openAuthSessionAsync(authUrl, redirectUri).then(
      async (response) => {
        if (response?.type === "success" && response?.url) {
          const { accessToken, refreshToken, idToken } = getTokens(
            response.url,
          );
          ...
          await UserProvider.setUser(user).then(() => signIn(user));
        }
      },
    );
  };
  ...
  const handleAuthenticationLogout = async () => {
    const user = await UserProvider.getUser().then(
      (user: User) =>
        (urlWithHeaders =
          urlWithHeaders + `?${"Authorization"}=${user.accessToken}`),
    ); // encodeURIComponent
