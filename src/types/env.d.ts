declare module "@env" {
  export const DOTTY_BASE_URL: string;
  export const DOTTY_WS_URL: string;
  export const CHSH_AUTHORIZATION_ENDPOINT: string;
  export const CHSH_AUTHORIZATION_KEY: string;
  export const CHSH_AUTHORIZATION_REDIRECT_URI: string;
  export const ALPACA_AUTHORIZATION_ENDPOINT: string;
  export const AUTH0_CLIENT_ID: string;
  export const AUTH0_CLIENT_SECRET: string;
  export const AUTH0_DISCOVERY_URI: string;
  export const AUTH0_REFRESH_URI: string;
  export const AUTH0_SIGNOUT_URI: string;
}
